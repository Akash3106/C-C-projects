...
void enqueue()
{
    ...
    if ((temp->burstTime <= 0) || (temp->priority < 0))
    {
...
void find()
{
    ...
    while (temp != NULL)
    {
        ...
        {
            if ((chosen->priority > temp->priority) && (temp->priority <= totalTime))
            {
                chosen = temp;
            }
        }
...
int main()
{
   ...
   for (i = 0;i < nOfPr;i++)
   {
    ...
    printf("\n\nProcess\tWaiting time\tTurnaround time\tended at\n");
    while (start != NULL)
    {
        ...
        else
        {
            ...
            chosen->turnaround = totalTime - chosen->priority;
